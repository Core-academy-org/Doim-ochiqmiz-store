...
export const MainStoreMapCard: React.FC<MainStoreMapCardProps> = ({
  ...
  const activeMapUrl = mapUrl || MAIN_STORE_GOOGLE_MAP_URL;
  const activeBranchName = branchName || t.mainStoreBranchName;
  const activeAddress = address || t.mainStoreAddress;

  const handleCopyAddress = () => {
    soundFx.playClick('chime');
    navigator.clipboard.writeText(`${activeBranchName}: ${activeAddress} - ${activeMapUrl}`);
    setCopied(true);
        ...
        <div className="lg:col-span-7 space-y-5">
          ...
          <div className="space-y-2 text-xs text-slate-200 bg-white/5 p-4 rounded-2xl border border-white/10 backdrop-blur-md">
            <div className="flex items-start gap-2.5">
              ...
              <span className="font-medium">{activeAddress}</span>
            </div>
          ...
          >
            ...
            <div className="relative aspect-16/10 w-full overflow-hidden bg-slate-800">
              ...
              <div className="absolute inset-0 flex flex-col items-center justify-center p-4 text-center">
                ...
                <span className="px-3 py-1 bg-slate-950/90 text-white rounded-full text-[11px] font-black border border-emerald-400/50 backdrop-blur-md shadow-lg max-w-[220px] truncate">
                  📍 {activeBranchName}
                </span>
            ...
            <div className="p-3 bg-slate-950/90 backdrop-blur-md flex items-center justify-between text-xs border-t border-white/10">
              <span className="text-slate-300 font-medium text-[11px] flex items-center gap-1 truncate max-w-[200px]">
                <Navigation className="w-3.5 h-3.5 text-emerald-400 shrink-0" />
                <span className="truncate">{activeMapUrl}</span>
              </span>
              <span className="text-emerald-400 font-bold text-[11px] underline group-hover/map:text-emerald-300 shrink-0">
